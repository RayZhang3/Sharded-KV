[MIT 6.824](https://pdos.csail.mit.edu/6.824/) 是MIT开设的一门研究生级别的分布式系统课程。该课程深入探讨分布式系统的设计和实现原理，涵盖主题包括容错机制、共识算法、数据复制、一致性模型、分布式存储系统以及 MapReduce等。通过讲座、论文阅读和编程，内容主要包括如何构建高性能、可扩展且可靠的分布式系统, 培养分布式系统领域的理论知识和实际技能。
# ShardKV
